...
export function SearchApp() {
  ...
  // Keyboard paths — ui_plan.md §8: "/" focuses input, ↑↓ moves the record list, Enter
  // opens a record, Esc closes the drawer (handled inside EvidenceDrawer itself).
  useEffect(() => {
    function onKeyDown(e: KeyboardEvent) {
      ...
      if (e.key === "/" && !typing) {
        ...
        e.preventDefault();
        setFocusIndex((i) => Math.min(i + 1, state.records.length - 1));
      } else if (e.key === "ArrowUp" && !typing && state.records.length > 0) {
        e.preventDefault();
        setFocusIndex((i) => Math.max(i - 1, 0));
      } else if (e.key === "Enter" && !typing && focusIndex >= 0 && state.records[focusIndex]) {
        openEvidence(state.records[focusIndex].record_id);
      }
    }
    document.addEventListener("keydown", onKeyDown);
    return () => document.removeEventListener("keydown", onKeyDown);
  }, [state.records, focusIndex]);
                ...
                {(state.recordsLoading || state.records.length > 0) && (
                  <div className="mt-6 space-y-3">
                    <h2 className="mono mb-3 text-xs uppercase tracking-wide text-[var(--text-low)]">
                      Records {state.candidateCount != null && <span>· {state.candidateCount} candidates</span>}
                    </h2>
                    {state.recordsLoading
                      ? Array.from({ length: 3 }).map((_, i) => <RecordCardSkeleton key={i} variant="main" />)
                      : state.records.map((r, i) => (
                          <RecordCard key={r.record_id} record={r} variant="main" onOpen={openEvidence} focused={i === focusIndex} />
                        ))}
                  </div>
                )}
              </>
