"""Serper.dev client — backs `web_search` (Google SERP via google.serper.dev).

This replaced the self-hosted OrioSearch/SearXNG backend entirely: SearXNG returned
non-empty but irrelevant results on long-tail entity queries (verified live: family-office
names came back as Japanese Q&A pages, embroidery shops, math calculators), which is fatal
for an obscure-entity qualification pipeline, and OrioSearch's local Docker container was a
single point of failure that took down a whole batch when it went down. Serper is a cloud
API with no local process to keep alive.

Page fetching is NOT here any more. `fetch_page` moved to app/tools/crawl.py (Crawl4AI,
headless Chromium) and Serper's paid scrape endpoint is no longer used anywhere — this
module is search-only now.

    web_search: {"results": [{"title","url","content","score"}], "query": str}

Degrades to an error shape on failure (missing key, HTTP error, JSON error) rather than
raising — callers must be able to treat could_not_verify uniformly and never have a lane
crash. When SERPER_API_KEY is unset it degrades to an empty/error result with no HTTP
call. Multi-key rotation handles credit exhaustion; see app/tools/keyrotation.py for why
the trigger is not simply a 429."""
