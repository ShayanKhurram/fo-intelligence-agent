...
class LinkedInPeopleProfileSpider(scrapy.Spider):
    """Patched for FO_Intelligence_Agent: the upstream spider hardcoded a single demo
    profile ('reidhoffman'). This version takes the profile via `-a profile=<slug-or-url>`
    (comma-separated for multiple), e.g.:
        scrapy crawl linkedin_people_profile -a profile=reidhoffman
        scrapy crawl linkedin_people_profile -a profile=https://www.linkedin.com/in/reidhoffman/
    """
    ...
    def __init__(self, profile=None, *args, **kwargs):
        ...
        if not profile:
            raise ValueError("linkedin_people_profile requires -a profile=<slug-or-url>")
        self.profile_list = [self._to_slug(p.strip()) for p in profile.split(',') if p.strip()]
